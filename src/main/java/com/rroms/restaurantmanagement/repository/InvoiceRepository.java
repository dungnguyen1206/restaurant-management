...
public interface InvoiceRepository extends JpaRepository<Invoice, Long> {
    ...
    @Query(value = " select day(i.paid_at) as dayNumber, " +
            " month(i.paid_at) as monthNumber, " +
            " year(i.paid_at) as yearNumber, " +
            " coalesce(sum(i.total), 0) as revenue " +
            " from invoices i " +
            " where i.paid_at >= :startDate and i.paid_at < :endDate " +
            " group by year(i.paid_at), month(i.paid_at), day(i.paid_at) " +
            " order by year(i.paid_at), month(i.paid_at), day(i.paid_at)", nativeQuery = true)
    List<RevenueTrendProjection> getRevenueTrendByDay(@Param("startDate") Instant startDate,
                                                      @Param("endDate") Instant endDate);

    @Query(value = " select month(i.paid_at) as monthNumber, " +
            " year(i.paid_at) as yearNumber, " +
            " coalesce(sum(i.total), 0) as revenue " +
            " from invoices i " +
            " where i.paid_at >= :startDate and i.paid_at < :endDate " +
            " group by year(i.paid_at), month(i.paid_at) " +
            " order by year(i.paid_at), month(i.paid_at)", nativeQuery = true)
    List<RevenueTrendProjection> getRevenueTrendByMonth(@Param("startDate") Instant startDate,
                                                        @Param("endDate") Instant endDate);

    @Query(value = " select year(i.paid_at) as yearNumber, " +
            " coalesce(sum(i.total), 0) as revenue " +
            " from invoices i " +
            " where i.paid_at >= :startDate and i.paid_at < :endDate " +
            " group by year(i.paid_at) " +
            " order by year(i.paid_at)", nativeQuery = true)
